...
#   define SNAKE_LONG 2
    ...
    typedef struct {
        map_tile_t tiles[MAX_Y][MAX_X]; int x_size; int y_size;
    } map_t;

    typedef enum {UP, DOWN, LEFT, RIGHT} direction_t;
    typedef struct { int x; int y; } snake_segment_t;
    typedef struct {
        snake_segment_t segments[MAX_LEN]; int size; direction_t direction;
    } snake_t;
